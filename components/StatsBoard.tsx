...
export const StatsBoard: React.FC<StatsBoardProps> = ({ stats }) => {
  const StatCard = ({ title, value, icon: Icon, colorClass, iconColor }: { title: string, value: string, icon: any, colorClass: string, iconColor: string }) => (
    <div 
      className="relative p-4 bg-[var(--card-bg)] border border-white/10 group hover:border-accent/50 transition-colors duration-300"
      style={{ clipPath: 'polygon(15px 0, 100% 0, 100% calc(100% - 15px), calc(100% - 15px) 100%, 0 100%, 0 15px)' }}
    >
      {/* Decorative corner accents */}
      <div className="absolute top-0 left-0 w-3 h-3 border-t border-l border-white/20 group-hover:border-accent transition-colors" />
      <div className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-white/20 group-hover:border-accent transition-colors" />

      <div className="flex items-center space-x-4 relative z-10">
        <div className={`p-3 bg-black/40 border border-white/10 shadow-[0_0_10px_rgba(0,0,0,0.5)]`}>
